...
bool PhysicsShapeCache::addShapesWithFile(const std::string &plist, float scaleFactor)
{
	...
	for (auto iter = bodydict.cbegin(); iter != bodydict.cend(); ++iter)
	{
		...
		bodyDef->isDynamic = true; //bodyData.at("is_dynamic").asBool();
		bodyDef->affectedByGravity = true; // bodyData.at("affected_by_gravity").asBool();
		bodyDef->allowsRotation = false; // bodyData.at("allows_rotation").asBool();
		bodyDef->linearDamping = 0; // bodyData.at("linear_damping").asFloat();
		bodyDef->angularDamping = 0; // bodyData.at("angular_damping").asFloat();
		bodyDef->velocityLimit = 0; // bodyData.at("velocity_limit").asFloat();
		bodyDef->angularVelocityLimit = 0; // bodyData.at("angular_velocity_limit").asFloat();
		...
		for (auto &fixtureitem : fixtureList)
		{
			...
			fd->tag = 0; // fixturedata.at("tag").asInt();
			fd->group = 0; //fixturedata.at("group").asInt();
			fd->categoryMask = 0; // fixturedata.at("category_mask").asInt();
			fd->collisionMask = 0; //fixturedata.at("collision_mask").asInt();
			fd->contactTestMask = 0; //fixturedata.at("contact_test_mask").asInt();
